...
PATH_MODEL, LOG_DIR, BATCH_SIZE, NUM_POINT, MAX_EPOCH, BASE_LEARNING_RATE, GPU_INDEX, MOMENTUM, OPTIMIZER, DECAY_STEP, DECAY_RATE, LOG_FOUT, NO_ROTATION,TRAINED_MODEL_PATH = args_to_flag(parser)
    ...
    with tf.Graph().as_default() as g:
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            with tf.variable_scope("transformation"):
                landmarks = MODEL.compute_descriptors(pointclouds_pl,  is_training_pl, N_BASIS*2,bn_decay=None, icp=False)
            ...
            descr_loss = losses.get_descriptors_loss(basisA, descrA,basisB, descrB)
            total_loss = descr_loss
        ...
        sess.run(init)
        saver_pretrain_basis_network.restore(sess, TRAINED_MODEL_PATH)
